...
def _normalize_outcome(outcome: dict[str, Any]) -> tuple[tuple[str, ...], str, int | None]:
    ...
    if any(bool(item.get("death_penalty")) for item in penalties):
        return charges, PenaltyType.DEATH.value, None
    if any(bool(item.get("life_imprisonment")) for item in penalties):
        return charges, PenaltyType.LIFE.value, None
    ...
    if months > 0:
        return charges, PenaltyType.FIXED_TERM.value, months
    if any(int(item.get("detention") or 0) > 0 for item in penalties):
        return charges, PenaltyType.DETENTION.value, None
    if any(int(item.get("surveillance") or 0) > 0 for item in penalties):
        return charges, PenaltyType.CONTROL.value, None
    return charges, PenaltyType.EXEMPT.value, None
        ...
        for line_number, line in enumerate(handle, start=1):
            ...
            try:
                ...
                for defendant_index, outcome in enumerate(outcomes):
                    charges, penalty_type, months = _normalize_outcome(outcome)
                    yield CaseUnit(
                        dataset="cmdl",
                        split=split,
